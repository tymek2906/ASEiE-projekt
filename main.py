...
def readwarc(pos):
    ...
    df = df.select(f.col('uri'), f.regexp_extract(f.col('value'),
                                                  """WARC-Type: response[\s\S]*?</html>""", 0).alias('text'))\
    ...
    df = df.select(f.regexp_replace(f.col('uri'), "WARC-Target-URI: ", '').alias('uri'), f.col('text'),
                   f.regexp_replace(f.col('date'), "(WARC-Date: )|T", '').alias('date'))
    return df


if __name__ == '__main__':
    df = readwarc('march2019.warc')
    pomdf = (df.select(f.explode(f.split(f.col('text'), ' ')).alias('word'),
                       f.col('date'))
             .where(f.length('word') > 0))
    x = pomdf.where(f.col('word') == "jest").groupBy('word', 'date').count().show()
    spark.stop()
